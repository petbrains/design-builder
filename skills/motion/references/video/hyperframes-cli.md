...
2. **Write** — author HTML composition (see [`hyperframes.md`](hyperframes.md))
...
Lints `index.html` and all files in `compositions/`. Errors must be fixed; warnings should be fixed; info appears with `--verbose`.

## Visual inspect

```bash
npx hyperframes inspect                 # default sweep
...
Errors should be fixed before rendering. `--strict` fails on warnings too. Mark intentional overflow with `data-layout-allow-overflow`; mark elements that should never be audited with `data-layout-ignore`.
...
Hot-reloads on file changes. Opens the studio. Hand back the studio URL, not the source path:
